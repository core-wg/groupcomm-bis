...
Similarly, if supported, reverse mapping (from IP multicast address to Group FQDN) is also possible, e.g., by using the reverse DNS resolution technique {{RFC1035}}. Reverse mapping is important, for example, in troubleshooting to translate IP multicast addresses back to human-readable hostnames to show in a diagnostics user interface.
