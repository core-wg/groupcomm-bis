...
An application group can be named in many ways through different types of identifiers, such as numbers, URIs or other strings. An application group name or identifier, if explicitly encoded in a CoAP request, is typically included in the path component or in the query component of a Group URI. It may also be encoded using the Uri-Host Option {{RFC7252}} in case that application group members implement a virtual CoAP server specific to that application group. The application group can then be identified by the payload of the Uri-Host Option and each virtual server serves one specific application group. However, encoding the application group in the Uri-Host Option is not the preferred method because in this case the application group cannot be encoded in a Group URI, and also the Uri-Host Option is being used for another purpose than encoding the host part of a URI as intended by {{RFC7252}} -- which is potentially confusing.
...
To create a security group, a configuring entity defines the security material - a set of group properties including the crytpographic algorithms and parameters used in the group, as well as additional information relevant throughout the group life-cycle, including the security group name and description. This task MAY be entrusted to a dedicated administrator, that interacts with a Group Manager as defined in {{chap-oscore}}. After that, security keys to protect group communications have to be generated (as part of the security material), as compatible with the specified group configuration.
