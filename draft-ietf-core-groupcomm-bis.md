...
For unsecured group communication (see {{chap-unsecured-groupcomm}}), addition/removal of CoAP group members is simply done by configuring these devices to start/stop listening to the group IP multicast address, and to start/stop the CoAP server listening to the group IP multicast address and UDP port.
...
* Group OSCORE prevents to make any single group member a target for subverting security in the whole OSCORE group (see Section 11.6 of {{RFC7252}}), even though all group members share (and can derive) the same symmetric key material used for encrypting messages sent to the OSCORE group (see Section 10.1 of {{I-D.ietf-core-oscore-groupcomm}}). In fact, countersignatures computed with a node's individual private key ensure source authentication of exchanged CoAP messages, as originated from the alleged, identifiable sender in the OSCORE group.
